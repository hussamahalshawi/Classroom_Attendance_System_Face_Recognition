...
cur = con.cursor()
cur.execute("CREATE TABLE IF NOT EXISTS IT (ID,name)")
...
while True:
        ...
        if face is not None:
            ...
            cur.execute("select * from IT where ID ='%s'" %student_ID)
            result = cur.fetchone()

            # print(result)
            a = cur.execute("select ID from IT")
            i = 0
            for idrow in a:
                if result is not None:
                    print(result[0])
                else:
                    cur.execute("INSERT INTO IT VALUES (?,?)", (student_ID, student_full_name))


                i +=1


            image_name_path = "./faces/students/" + student_full_name +\
                              "." + student_ID + "." + str(count) + ".jpg"
        ...
        if (cv.waitKey(1) & 0xFF == ord("q")) or count == 5:
            break
